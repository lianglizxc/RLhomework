...
class Agent(object):
    ...
    def policy_forward_pass(self, sy_ob_no):
        ...
        if self.discrete:
            ...
            sy_logits_na = build_mlp(sy_ob_no, self.ac_dim, "discrete_mlp", self.n_layers, self.size)
            return sy_logits_na
        else:
            # YOUR_CODE_HERE - 4. Problem 2(b)(ii)
    ...
    def build_computation_graph(self):
        ...
        if self.nn_baseline:
            self.baseline_prediction = tf.squeeze(build_mlp(
                ...
                size=self.size))
            # YOUR_CODE_HERE
            self.sy_target_n = tf.placeholder(dtype=tf.float32, shape = [None], name = "reward_esimate")
            self.baseline_loss = tf.reduce_mean(self.sy_target_n - self.baseline_prediction)
    ...
    def sum_of_rewards(self, re_n):
        ...
        if self.reward_to_go:
            ...
            q_n = np.concatenate(
                [[sum(re_tau[start:] * (self.gamma ** np.arange(0, len(re_tau) - start)+1)) \
    ...
    def update_parameters(self, ob_no, ac_na, q_n, adv_n):
        ...
        if self.nn_baseline:
            ...
            # YOUR_CODE_HERE
            target_n = (q_n - np.mean(q_n))/(np.std(q_n) + 0.001)
            loss_qn, _ = self.sess.run([self.baseline_loss, self.baseline_update_op], feed_dict={self.sy_ob_no : ob_no, self.sy_target_n : target_n })
