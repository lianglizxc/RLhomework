...
class Histogram(Density_Model):
    ...
    def get_prob(self, states):
        ...
        """
        all_counts = self.get_count(states)
        probs = all_counts / self.total
        return probs
...
class Exemplar(Density_Model):
    ...
    def build_computation_graph(self):
        ...
        b_probs = self.discriminator.probs
        self.log_likelihood = tf.log(b_probs) * self.discrim_target + (1 - self.discrim_target)*tf.log(1 - b_probs)
        self.log_likelihood = tf.squeeze(self.log_likelihood)

        self.likelihood = (b_probs ** self.discrim_target) * ((1 - b_probs) ** (1-self.discrim_target))
        self.likelihood = tf.squeeze(self.likelihood)
        ...
        self.elbo = tf.reduce_mean(self.log_likelihood - self.kl_weight * self.kl, axis=0)
    ...
    def make_encoder(self, state, z_size, scope, n_layers, hid_size):
        ...
        z_logstd = tf.Variable(initial_value=tf.zeros(shape = [z_size], dtype=tf.float32))
    ...
    def make_prior(self, z_size):
        ...
        """
        prior_mean = tf.zeros(shape = [z_size], dtype=tf.float32)
        prior_logstd = tf.zeros(shape = [z_size], dtype=tf.float32)
    ...
    def get_likelihood(self, state1, state2):
        ...
        likelihood = self.sess.run(self.likelihood,
                                   feed_dict = {self.discrim_target: np.ones([state1.shape[0], 1]),
                                                self.state1: state1, self.state2: state2})
